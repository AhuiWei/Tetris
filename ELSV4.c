...
}Table_board[Vertical_boxs][Horizontal_boxs];/*游戏方块结构*/

struct SHAPE
{
    char box[2];                     /*一个字节等于8位，没4位表示一个游戏方块的一行
...
{
    (*oldtimer)();     /*调用原来的中断服务程序*/
    TimerCounter++;    /*全局计数变量增加*/
}
...
void SetTimer(void interrupt(*IntProc)(void))
{
    ...
    /*获取中断号为TIMER的中断函数的入口地址*/
    disable();
    /*设置新的时钟中断处理过程时，禁止所有中断*/
    setvect(TIMER,IntProc);
    /*即将断号为TIMER的中断处理函数的入口地址为IntProc()函数的入口地址，即中断发生时，将调用IntProc()函数*/
    enable();
    /*开启中断*/
}
...
void ShowSpeed(int speed)
{
    ...
    y=150;
    bar(x-BSIZE,y,x+BSIZE*3,y+BSIZE*3);
    /*确定一个以(x1,y1)为左上角。(x2,y2)为右下角的举行窗口，再按规定图模和颜色填充。*/
...
int DelFullRow(int y)
{
    /*该行游戏板下移一行*/
    int n,top=0;
    /*top保存的是当前最高点，出现一行全部空就表示到达最高点了，移动是到最高点结束*/
    register m,total;
    for(n=y;n>=0;n--)/*从当前行往上看*/
    {
        ...
        for(m=0;m<Horizontal_boxs;m++)
        {
            ...
            if(Table_board[n][m].var!=Table_board[n-1][m].var)
                /*上行不等于下行就把上行传给下行，此处判断为程序优化部分，也可以不要判断，直接下移即可*/
            {
    ...
    */
    ...
    /*如果向左移*/
    {
        ...
        for(n=0;n<4;n++)
        {
            for(m=0;m<4;m++)
            {
                if((shapes[box_numb].box[n/2])&mask) /*因为box只有box[0]和box[1]两个元素，所以必须除以2*/
                {
                    if((x+BSIZE*m)<Sys_x) return (false);
                    /*碰到最左边了*/
                    else if(Table_board[t_boardy+n][t_boardx+m].var) /*左移一格游戏方块后，此4*4的区域与游戏板块有冲突*/
                    {
                        return (false);
                    }
                }
    ...
    else if(direction==MoveRight) /*如果向左移*/
    {
        x+=BSIZE;
        t_boardx=(x-Sys_x)/BSIZE;
        t_boardy=(y-Sys_y)/BSIZE;
        ...
        for(n=0;n<4;n++)
        {
            for(m=0;m<4;m++)
            {
                if((shapes[box_numb].box[n/2])&mask)
                {
                    if((x+BSIZE*m)>=(Sys_x+BSIZE*Horizontal_boxs)) return (false); /*碰到最左边了*/
        ...
        for(n=0;n<4;n++)
        {
                for(m=0;m<4;m++)
                {
                    ...
                    if((shapes[box_numb].box[n/2])&mask)
                        ...
                        mask=mask/(2);
                        /*mask依次为：10000000,01000000,00100000,00010000,00001000,00000100,00000010,00000001*/
                        if(mask==0)mask=128;
                }
        }
        if(flag_newbox)
...
void main()
{
    ...
    while(1)
    {
        ...
        if(flag_newbox)
        {
            /*这时相当于游戏方块已经到了底部了，把其中一行出现点满一行的清除，置0*/
